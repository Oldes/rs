rebol [
	...
	clear-line: func[][
		error? try [
			xdebug rejoin ["ib:" index? buffer " lb:" length? buffer " lp:" length? prompt]
			xdebug rejoin ["buf:" mold buffer]
			xdebug rejoin ["clear:" ((index? buffer) + (length? prompt) + (length? buffer))]
		]
		prin rejoin ["^(1B)[" ((index? buffer) + (length? prompt) + (length? buffer)) "D^(1B)[K" ]
	]
	xdebug: func[str][
