rebol [
	...
	history: [
		3.0.1 "oldes" ["added /ctcp user command"]
		3.0.0 "oldes" [
			"Built using the latest versions of rs-projects (networking, console-port, ident)"
		...
		2.0.1 "oldes" ["Using rs-projects so it's more systemic"]
	...
	Require: [
		rs-project 'console-port 
		rs-project 'error-handler
		rs-project 'networking 
	]
...
debug?: off
;ctx-irc/connect to-string read dns://rur "hmm"

ctx-irc/connect "irc.glassbilen.net" "hmm"
;ctx-irc/connect "localhost" "hmm"
;ctx-irc/connect "www.compkarori.co.nz" rejoin ["user" random 1000 ]
