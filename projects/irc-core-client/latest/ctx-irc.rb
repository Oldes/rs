	...
	parse-user-input: func[msg /local arg][
		...
		parse/all msg [
			...
			|	"/ctcp" some space copy arg some non-space some space copy msg to end (say/to rejoin ["^A" msg #"^A"] arg )
	...
	command-actions: make block! [
        ...
        "433" [cprint/err [ params/2 " - " params/3] ask-for-nickname] ;ERR_NICKNAMEINUSE
		...
		"251" [
			...
			use [ch][
				if all [
					...
					irc-port-send rejoin [
						"JOIN "
						either is-channel? ch [ch][join #"#" ch]
					]
				]
				;ud/on-connect
			]
		...
		"333" [
			use [a b][
				parse/all form params/3 [copy a to "!" 1 skip copy b to end]
				cprint/inf ["Topic for " params/2 " set by " a " (" b ") " 1-1-1970/0:0:0 + to-time to-integer params/4]
			]
		];RPL_TOPIC_setBy
	...
	connect: func[host nick /with settings /channel chan /local irc-port spec][
		error? try [ctx-ident/start/stop 0:0:20]
		host: parse form host ":"
		active-irc: current-port: open/lines/direct/no-wait compose/deep [
			scheme: 'tcp
			host: (first host)
			port-id: (either all [1 < length? host not error? try [to-integer host/2]] [to-integer host/2][6667])
			user-data:  make object! [
				user: ( copy/part system/network/host 5 )
				nick: ( nick )
				ident: ( join "Reb_" system/user/name )
				on-connect: none
				channels: make block! []
				away: none
				start-channel: ( either all [channel string? chan][chan]["#rebol"] )
