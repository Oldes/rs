	...
	chat-parser: func[msg chat-rules /local err tmp][
		foreach [locals rule action] current-port/user-data/chat-rules [
			use locals [
				if parse/all msg bind rule 'self [try-do bind action 'say ]
			]
	    ]
	]
	...
	command-actions: make block! [
		...
		"333" [cprint/inf ["Topic set by " params/2 " " 1-1-1970/0:0:0 + to-time to-integer params/3]];RPL_TOPIC_setBy
	...
	connect: func[host nick /with settings /local irc-port spec][
		...
		active-irc: current-port: open/lines/direct/no-wait compose/deep [
			...
			user-data:  make object! [
				user: ( join nick random 1000 )
				nick: ( nick )
				ident: ( join "Reb_" random "abcdefghijklmnopqrstuvwxyz" )
		...
		add-port active-irc :port-handler
