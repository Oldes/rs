	...
	import-media-img: func[
		...
		with ctx-imagick [
			...
			unless zero? MagickReadImage *wand utf8/encode probe to-local-file source-file [
				print "aaaaa"
				either find ["JPG" "JPEG"] imgFormat: probe MagickGetImageFormat *wand [
					write/binary xfl-folder/bin/:dat-file read/binary source-file
				][
					...
					with dat-io [
						...
						writeBytes #{0000}
						write/binary xfl-folder-new/bin/:dat-file head outBuffer
					]
					if dom [
						...
						unless find Media name [
							...
							insert/only Media-content reduce [
								"DOMBitmapItem" tmp: reduce [
									...
									"allowSmoothing" either smVal = "true" ["true"]["false"]
									"frameRight"  20 * width
...
comment {
	ARGB-to-img: func[ARGB size /local img r RGB ARGB sk cols rows][
		img: make image! size
		...
		cols: size/x
		rows: size/y
		probe sk: either 0 = r: cols // 2 [cols][cols + 2 - r]
		
		loop rows [
			loop cols [
				RGB:   change RGB copy/part next ARGB 3
				alpha: change alpha first ARGB
				ARGB: skip ARGB 4
			]
			;RGB: skip RGB sk
			ARGB: skip ARGB sk
	...
	export-media-item: func[
		...
		print "--"
		if all [
			probe exists? probe xfl-folder/bin/(item/("bitmapDataHRef"))

			any [
				(
					probe ext-file: join xfl-folder-new to-rebol-file item-file
					overwrite
				)
				not exists? ext-file
			]
				...
				print ".."
				with dat-io [
					setStreamBuffer read/binary xfl-folder/bin/(item/("bitmapDataHRef"))
								...
								with ctx-imagick [
									start
										...
										unless all [
											...
											not zero? MagickWriteImages *wand probe to-local-file ext-file
										][
											errmsg: reform [
												Exception/Severity "="
												ptr-to-string desc: MagickGetException *wand Exception
	...
	export-sound: func[
		...
		probe item
		probe dat-file: join xfl-folder [%bin/ item/("soundDataHRef")] 
		probe sz: size? dat-file
		wav-file: join xfl-folder-new [%LIBRARY/ item/("name")]
