...
parse-SHAPE: func[/withstyle /glyph "used for parsing glyph shapes" /local fills bits NumBits points b shapes linestyles][
	...
	while ["000000" <> copy/part bits 6 ][
		either bits/1 = #"0" [
			...
			if states/2 = #"1" [
				;print "Line style change flag"
				LineStyle: UB-to-int copy/part bits NumBits/2
				bits: skip bits NumBits/2
				;if not empty? points [print ["!!!POINTS:" points]]
				if states/1 <> #"1" and not none? glyph [
					insert points either LineStyle = 0 [
						[no edge]
					][
...
parse-DefineFont2: func[
	/local id flags NameLen glyphs OffsetTable wideOfs ofsFST last-ofs
	GlyphShapes codes Advances Bounds
][
	ind+
	...
	font/ANSI:        (flags/4 = #"1")
	font/WideOffsets: (flags/5 = #"1")
	font/WideCodes:   (flags/6 = #"1")
	font/italic:      (flags/7 = #"1")
	font/bold:        (flags/8 = #"1")
	
	;print [tabs "Font ID:" id]
	;print [tabs "Flags:" flags]

	font/LanguageCode: to-integer tag-bin-part 1

	NameLen: to-integer tag-bin-part 1

	font/name: to-string tag-bin-part NameLen
	glyphs: to-integer tag-bin-part/rev 2
	...
	codeOffset: to-integer tag-bin-part/rev wideOfs
	;print [tabs "CodeOffset:" codeOffset]
	append OffsetTable codeOffset - ofs
	;print [tabs "OffsetTable:" OffsetTable]
	font/Glyphs: make block! (2 * glyphs)
	GlyphShapes: make block! glyphs
	Advances: make block! glyphs
	Bounds: make block! glyphs
	
	last-ofs: 0
	foreach ofs OffsetTable [
		append GlyphShapes tag-bin-part (ofs - last-ofs)
		last-ofs: ofs
	]
	
	Codes: tag-bin-part (glyphs * wideOfs)

	if flags/1 = #"1" [
		Font/Ascent:  SB-to-int tag-bin-part 2
		Font/Descent: SB-to-int tag-bin-part 2
		Font/Leading: SB-to-int tag-bin-part 2

		loop glyphs [
			insert tail Advances SB-to-int tag-bin-part 2
		]
		loop glyphs [
			append/only Bounds get-rect/integers enbase/base copy/part tag-bin 16 2
			tag-bin: skip tag-bin (extend-int skip-val) / 8
		]
		font/Kerning: to-integer tag-bin-part/rev 2
	]
	;if not empty? tag-bin [
	;	print [tabs "..." tag-bin]
	;]
	print [length? GlyphShapes length? Advances]
	either (length? GlyphShapes) = (length? Advances) [
		foreach glyph GlyphShapes [
			tag-bin: copy glyph
			insert tail font/glyphs reverse copy/part Codes wideOfs
			insert/only tail font/glyphs reduce [
				first parse-shape
				first Advances
				first Bounds
			]
			remove/part Codes wideOfs
			remove Advances
			remove Bounds
			;print "--"
		]
	][
		print "missing advance values!"
	]
	ind-
	font
]