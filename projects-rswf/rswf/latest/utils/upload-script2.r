...
upload-rswf-script: func[rswf-file [file!] /box /local data swf-version rswfdata][
	...
	write/binary %uploadtemp.swf make-swf/compressed rswf-file 
	rswfdata: read rswf-file
	
	either exists? %uploadtemp.swf [
		...
		print read/custom rswf-web-url/submit2.php compose/deep [
			multipart [
				type         "example"
				name         (name)
				title        (form either error? try [header/title][name][header/title])
				author       (form either error? try [header/author][""][header/author])
				email        (form either error? try [header/email ][""][header/email ])
				...
				swfrate      (header/rate)
				created      (to-sql-date either none? header/date [modified? rswf-file][header/date ])
				modified     (to-sql-date modified? rswf-file)
				purpose      (form either none? header/purpose [""][header/purpose])
				comment      (form either none? header/comment [""][header/comment])
				jscode       (form either error? try [header/js ][""][header/js])
				related      ""
				swf			 %uploadtemp.swf
				code         (rswfdata)
				colorized    (clrdata)
		...
		;delete %uploadtemp.swf
	][
		print ["!!!! not uploading rswf:" rswf-file ]
	]
