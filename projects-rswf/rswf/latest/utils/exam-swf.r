...
read-longFloat: func["reads 4 bytes and converts them to decimal!" /local tmp][
	tmp: tag-bin-part 4
	from-ieee32 join 
		reverse copy/part tmp 3
		copy/part skip tmp 3 1
]
read-ui32:   func[][to integer! tag-bin-part/rev 4]
read-short:  func[][to integer! tag-bin-part/rev 2]
read-byte:   func[][tag-bin-part 1]
read-tuple:  func[][to tuple!   tag-bin-part 4]
read-string: has[r][
	i: index? find tag-bin #{00}
	r: as-string tag-bin-part (i - index? tag-bin)
	tag-bin-part 1
	r
]
...
read-U30integer: has[r b s][
	b: first tag-bin tag-bin: next tag-bin
	if b < 128 [return b]
	r: b and 127
	s: 128
	while [b: first tag-bin tag-bin: next tag-bin][
		r: r + (b * s)
		if 128 > b [return r]
		s: s + 128
	]
...
parse-SHAPE: func[/withstyle /local fills bits NumBits points b][
	probe copy/part tag-bin 5
	if withstyle [
		parse-FILLSTYLEARRAY
		parse-LINESTYLEARRAY
	]
	print ["????:" mold copy/part tag-bin 5]
	...
	prin "??" probe copy/part tag-bin 10
...
parse-LINESTYLEARRAY: func[][
	print [tabs "LineStyleCount:" lines: get-count  mold copy/part tag-bin 10]
	if tag-bin/1 > 0 [
		ind+
