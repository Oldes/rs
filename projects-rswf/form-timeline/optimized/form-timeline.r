...
ctx-form-timeline: context [
	...
	form-sprite-tags: func[
		...
		/local
			...
			depths-replaced depths-to-remove ids-at-depth currentFrame
			name numLabels
	][
		...
		foreach tag tags [
			...
			switch/default tagId [
				26 70 [;placeObject
					...
					;print ["placeObject: " depth move cid attributes]
					if integer? cid [cid: cid + id-offset]

					insert tail used-depths depth
					name: tagData/7
				...
				1 [;showFrame
					used-depths: unique used-depths
					;print ["-- ShowFrame"]
		...
		used-depths: sort used-depths
		if numLabels > 0 [
			insert find/tail head result #"^/" "^-HasLabels^/"
		]
		append usage-counter id
		repend types [id 'object]
		;result: head result ;optimize-controlTags 
		result: optimize-depths head result used-depths

		repend sprites [id reduce [frames result]]
	]
	optimize-depths: func [
		data [string!]
		used-depths [block!]
		/local result currentFrame cmd depth transform color type id name pos
	][
		print ["optimize-depths" mold used-depths]
		result: make string! length? data
		currentFrame: 1

		parse/all load data [
			'TotalFrames set frames integer! (
				result: insert result ajoin ["^-TotalFrames " frames " MaxDepth " length? used-depths lf]
			)
			opt ['HasLabels (result: insert result "^-HasLabels^/")]
			any [
				pos:
				'Move set depth integer! set transform block! set color [block! | none] (
					...
					result: insert result rejoin [
						"^-Move "
						index? find used-depths depth #" "
						mold/all transform
						either color [mold/all color][""]
						#"^/"
					]
				)
				|
				...
				(
					result: insert result rejoin [
						...
						#" " mold/all transform
						either color [mold/all color][""]
						either name  [mold name][""] 
						#"^/"
					]
				)
				|
				'Remove set depth integer! (
					result: insert result ajoin ["^-Remove " index? find used-depths depth "^/"]
				)
				|
				'Label set name string! (
					result: insert result ajoin ["^-Label " mold name #"^/"]
				)
				|
				'Sound set id integer! set soundData block! (
					result: insert result ajoin ["^-Sound " id #" " mold soundData #"^/"]
				)
				| pos: 1 skip (
					ask reform ["(optimize-depth) UNKNOWN COMMAND near:" mold copy/part pos 20 "..."] 
				)
			]
		]
		probe head result
	]
	optimize-controlTags: func [
		data [string!]
		/local
			;result to-remove to-add to-move at-stage removing depth-offset
			frame frames depth transform color type id name pos
	][
		...
		parse/all load data [
			'TotalFrames set frames integer! (
				result: insert result ajoin ["^-TotalFrames " frames lf]
			)
			opt ['HasLabels (result: insert result "^-HasLabels^/")] 
			any [
				pos:
				'Move set depth integer! set transform block! set color [block! | none] (
					...
					repend/only to-move [depth transform color]
				)
				|
				'ShowFrame (
					print "==ShowFrame=="

					print ["R" mold to-remove]
					print ["M" mold to-move]
					print ["A" mold to-add]

					frame: frame + 1
					clear removing
					foreach depth to-remove [
						;print ["to-remove" depth]
						;if at-stage/(depth) [
							append/only removing at-stage/(depth)
							at-stage/(depth): none
						;] 
					]
					probe new-line/all at-stage true
					print "--"
					forall to-add [
						set [type id depth transform color name] to-add/1
						replacing?: false
						forall to-remove [
							if depth = to-remove/1 [
								remove to-remove
								replacing?: true
								break
							]
						]
						to-remove: head to-remove

						either depth > length? at-stage [
							append/only at-stage to-add/1
						][	at-stage/(depth): to-add/1 ]

						result: insert result rejoin [
							#"^-" either replacing? ["Replace "]["Place "] type #" " id #" " depth #" "
							mold/all transform
							either color [mold/all color][""]
							either name [mold name][""]
							#"^/"
					...
					maxDepth: 0
					forall at-stage [if at-stage/1 [maxDepth: maxDepth + 1]]
					at-stage: head at-stage

					depth: 0
					;probe new-line/all at-stage true

					until [
						while [all [not tail? at-stage at-stage/1]][
							at-stage: next at-stage
						]
						depth: index? at-stage
						if tail? at-stage [break] 
						while [all [not tail? at-stage none? at-stage/1]][
							index: index? at-stage
							;print [index maxDepth]
							if index < maxDepth [
								result: insert result rejoin [
									"^-Dispose " index #"^/"
								]
						...
						while [all [not tail? at-stage at-stage/1]][
							result: insert result rejoin [
								"^-ChangeDepth " index? at-stage #" " depth #"^/"
							]
							poke head at-stage depth at-stage/1
							at-stage/1: none
							depth: depth + 1
							at-stage: next at-stage
						]
					...
					forall to-move [
						set [depth transform color] to-move/1
						result: insert result rejoin [
							"^-Move "
							depth #" "
							mold/all transform
					...
					at-stage: tail at-stage
					if none? pick at-stage -1 [
						while [all [not head? at-stage none? at-stage/1]][
							at-stage: back at-stage
						]
						;probe new-line/all at-stage true
						depth: either any [empty? at-stage all [head? at-stage none? at-stage/1]][0][index? at-stage]
						result: insert result rejoin ["^-NewMaxDepth " depth #"^/"]
						clear at at-stage depth + 1
					]
					at-stage: head at-stage
					clear to-remove
					clear head to-move
					...
					result: insert result ajoin ["^-ShowFrame ;" frame #"^/"]
				)
				|
				'Place 
					set type word!
					...
					set name [string! | none]
				(
					;print ["Place: " type "with id" id "to" depth]

					repend/only to-add [type id depth transform color name]
					;repend/only at-stage [type id depth transform color name]

				)
				|
				'Replace set type word! set id integer! set depth integer! set transform block! set color [block! | none] (
					;print ["Replace: " type "with id" id "in" depth]

					result: insert result rejoin [
						"^-Replace " type #" " id #" " depth #" "
						mold/all transform
						either color [mold/all color][""]
						#"^/"
					]
				...
				'Remove set depth integer! (
					...
					repend to-remove [depth + depth-offset]
				...
				| pos: 1 skip (
					ask reform ["(optimize) UNKNOWN COMMAND near:" mold copy/part pos 20 "..."] 
				)
			]
		]
		head result
