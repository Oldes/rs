...
ctx-triangulator: context [
		...
		parse strokeDefinition [
			any [
				...
				pos: (
					...
					vertice-struct/x: v1xPos
					vertice-struct/y: v1yPos
					vertice-struct/r: clrR
					vertice-struct/g: clrG
					vertice-struct/b: clrB
					vertice-struct/a: clrA

					vertBin: third vertice-struct
					unless ind2: select vertToIndHash vertBin [
						ind2: numVertices
						insert vertToIndHash reduce [copy vertBin ind2]
						insert tail vertices vertBin
						numVertices: numVertices + 1
					]

					vertice-struct/x: v1xNeg
					vertice-struct/y: v1yNeg
					vertice-struct/r: clrR
					vertice-struct/g: clrG
					vertice-struct/b: clrB
					vertice-struct/a: clrA
					...
					vertBin: third vertice-struct
					unless ind3: select vertToIndHash vertBin [
						ind3: numVertices
						insert vertToIndHash reduce [copy vertBin ind3]
						insert tail vertices vertBin
						numVertices: numVertices + 1
					]
					unless isFirst? [
						repend indices [ind0 ind2 ind1 ind1 ind2 ind3]
					]
					ind0: ind2
					ind1: ind3
	...
	set 'triangulate-shape func[
		...
		/local
			...
			penPosX penPosY
			startIdx newHash vertBin
	][
		;print ["triangulate-shape" mold copy/part data 10 "..."]
		startIdx: length? indices
		startVert: numVertices
		...
		;probe new-line/skip to block! vertToIndHash true 2
		if numVertices > 65535 [;65535
			print ["BufferFull" length? vertices "startIdx: " startIdx " triangles: " ((length? indices) - startIdx) / 3 ]


			append/only indexBuffers  copy/part indices startIdx
			append/only vertexBuffers copy/part vertices (startVert * 24)

			indices: skip indices startIdx
			clear vertices

			newHash: make hash! 20000
			numVertices: startIdx: startVert: 0
			;probe new-line/skip to block! vertToIndHash true 2
			foreach ind indices [
				vertBin: first back find vertToIndHash ind
				unless ind: select newHash vertBin [
					ind: numVertices
					insert newHash reduce [copy vertBin ind]
					insert tail vertices vertBin
					numVertices: numVertices + 1
				]
				insert tail newIndices ind
			]

			indices: clear head indices
			insert indices newIndices
			clear newIndices
			insert clear vertToIndHash newHash
		]

		reduce [
			length? vertexBuffers                  ;-- indexBuffer
			startIdx                               ;-- firstIndex
			((length? indices) - startIdx) / 3     ;-- numTriangles
		]
	]
	...
	get-buffers-binary: func[][
		...
		either empty? vertexBuffers/1 [
			...
			foreach vertices vertexBuffers [
				out/writeUI32 (length? vertices) / 24
				print ["VERTEX BUFFER BYTES: " length? vertices]
				out/writeBytes vertices
				clear head vertices
			]
			;shapes indexBuffer:
